...
namespace fast_planner {
    ...
    void SDFMap::initMap(ros::NodeHandle &nh, std::shared_ptr<clutter_hand::CH_RC_SDF> rc_sdf_ptr) {
        ...
        nh.param("/drone0/planning/sdf_map/resolution", mp_->resolution_, -1.0);
        nh.param("/drone0/planning/sdf_map/map_size_x", x_size, -1.0);
        nh.param("/drone0/planning/sdf_map/map_size_y", y_size, -1.0);
        nh.param("/drone0/planning/sdf_map/map_size_z", z_size, -1.0);
        nh.param("/drone0/planning/sdf_map/obstacles_inflation", mp_->obstacles_inflation_, -1.0);
        nh.param("/drone0/planning/sdf_map/local_bound_inflate", mp_->local_bound_inflate_, 1.0);
        nh.param("/drone0/planning/sdf_map/local_map_margin", mp_->local_map_margin_, 1);
        nh.param("/drone0/planning/sdf_map/ground_height", mp_->ground_height_, 1.0);
        nh.param("/drone0/planning/sdf_map/default_dist", mp_->default_dist_, 5.0);
        nh.param("/drone0/planning/sdf_map/optimistic", mp_->optimistic_, true);
        nh.param("/drone0/planning/sdf_map/signed_dist", mp_->signed_dist_, false);
        ...
        nh.param("/drone0/planning/sdf_map/p_hit", mp_->p_hit_, 0.70);
        nh.param("/drone0/planning/sdf_map/p_miss", mp_->p_miss_, 0.35);
        nh.param("/drone0/planning/sdf_map/p_min", mp_->p_min_, 0.12);
        nh.param("/drone0/planning/sdf_map/p_max", mp_->p_max_, 0.97);
        nh.param("/drone0/planning/sdf_map/p_occ", mp_->p_occ_, 0.80);
        nh.param("/drone0/planning/sdf_map/max_ray_length", mp_->max_ray_length_, -0.1);
        nh.param("/drone0/planning/sdf_map/virtual_ceil_height", mp_->virtual_ceil_height_, -0.1);
        ...
        // info_map
        // md_->loss_buffer_all_ = vector<double>(buffer_size, 1.0);
        md_->loss_buffer_all_ = vector<double>(buffer_size, 0.0);
        md_->loss_buffer_temp_ = vector<double>(buffer_size, 0.0);
        md_->loss_cnt_buffer_ = vector<int>(buffer_size, 0);
        ...
        for (Eigen::Index i = 0; i < 3; ++i) {
            nh.param("/drone0/planning/sdf_map/box_min_" + axis[i], mp_->box_mind_[i], mp_->map_min_boundary_[i]);
            nh.param("/drone0/planning/sdf_map/box_max_" + axis[i], mp_->box_maxd_[i], mp_->map_max_boundary_[i]);
        }
        ...
        vis_ptr_ = std::make_shared<visualization_map::Visualization>(nh);
        ...
        nh.param("/drone0/planning/sdf_map/aabb_x", mp_->aabb_box_(0), 1.0);
        nh.param("/drone0/planning/sdf_map/aabb_y", mp_->aabb_box_(1), 1.0);
        nh.param("/drone0/planning/sdf_map/aabb_z", mp_->aabb_box_(2), 1.0);
    ...
    void SDFMap::inputPointCloud(
        ...
        for (size_t i = 0; i < point_num; ++i) {
            ...
            pt_w << pt.x, pt.y, pt.z;
            // std::cout << "[" << i << "] pt_w: " << pt_w.transpose() << std::endl;
            int tmp_flag;
            // Set flag for projected point
            if (!isInMap(pt_w)) {
                // 不在地图的放到max_ray_length_
...
// TODO not checked
bool SDFMap::checkRayValid(const Eigen::VectorXd& s0, const Eigen::VectorXd& s1)
{
    Eigen::Vector3d p0 = s0.head(3), p1 = s1.head(3);
    int n_samples = ceil((p0-p1).norm()/mp_->resolution_);

    // determine yaw direction
    double yaw0 = s0(3), yaw1 = s1(3);
    if(yaw1 - yaw0 > M_PI)
        yaw1 -= 2*M_PI;
    else if(yaw1 - yaw0 < -M_PI)
        yaw1 += 2*M_PI;

    Eigen::VectorXd ds = (s1-s0)/(double)n_samples;
    double d_yaw = (yaw1 - yaw0)/(double)n_samples;

    for(int i=0; i<n_samples; ++i){
        Eigen::VectorXd s = s0 + i*ds;
        s(3) = yaw0 + i*d_yaw;

// debug
// std::cout << "[rayValid] s(" << i << "): " << s.transpose() << std::endl;
// visualization_msgs::MarkerArray marker_array;
// rc_sdf_ptr_->getRobotMarkerArray(s, marker_array);
// rc_sdf_ptr_->robotMarkersPub(marker_array,"checkRayValid_debug");
...
//     for (int i = 0; i < 3; ++i) {
//       t_max(i) = step(i) > 0 ? (idx0(i) + 1) - p0(i) / getResolution() : p0(i) / getResolution() - idx0(i);
//     }
...
//         Eigen::Vector3d rayPt;
//         indexToPos(rayIdx,rayPt);
//         ray_pts.push_back(rayPt);
//         vis_ptr_->visualize_pointcloud(ray_pts, "checkRayValid");
//         ros::Duration(0.1).sleep();
//     }
//     return true;
//   }
...
void SDFMap::getAABBPointsSample(std::vector<Eigen::Vector3d>& aabb_pts, 
    ...
    for(size_t i=0; i<sample_pts.size(); ++i){
        ...
        Eigen::Vector3d min_cut_p, max_cut_p;
        for(int j=0; j<3; ++j){
            min_cut_p(j) = sample_pts[i](j)-0.5*mp_->aabb_box_(j);
            max_cut_p(j) = sample_pts[i](j)+0.5*mp_->aabb_box_(j);
        }
...
void SDFMap::getAABBPoints(Eigen::MatrixXd& aabb_pts, 
    ...
    for(int i=0; i<3; ++i){
        min_cut_p(i) = std::min(start_pos(i), end_pos(i))-0.5*mp_->aabb_box_(i);
        max_cut_p(i) = std::max(start_pos(i), end_pos(i))+0.5*mp_->aabb_box_(i);
    }
