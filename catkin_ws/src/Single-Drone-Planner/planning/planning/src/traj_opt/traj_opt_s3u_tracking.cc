...
// Generate MincoS3-uniform trajectory for given traget sequence within certain prediction time
...
inline double objectiveFuncTracking(void* ptrObj,
  ...
  const double& deltaT = x[0];
  double& grad_deltaT = grad[0];
  ...
  obj.cost_t_rec_ = obj.rhoT_ * deltaT * deltaT;
  obj.deltaT_rec_ = deltaT * deltaT;
  grad_deltaT = (obj.minco_s3u_opt_.gdT/obj.N_ + obj.rhoT_) * 2 * deltaT;
...
void TrajOpt::addTimeCostTracking(double& cost){
  const auto& T = minco_s3u_opt_.t(1);
  ...
  double gradViola_t;
  // exact term cost and gradient
  double cost_tracking_p;
  Eigen::Vector3d grad_tracking_p;
  ...
  for (int i = 0; i < M; ++i) {
    ...
    const auto& c = minco_s3u_opt_.b.block<6, 3>(piece * 6, 0);
    pos = c.transpose() * beta0;
    vel = c.transpose() * beta1;
    grad_p.setZero();
    grad_v.setZero();
    ...
    gradViola_c = beta0 * grad_p.transpose();
    gradViola_t = grad_p.transpose() * vel;
    minco_s3u_opt_.gdC.block<6, 3>(piece * 6, 0) += rho * step * gradViola_c;

    minco_s3u_opt_.gdT += -rho * piece * step * gradViola_t;
...
bool TrajOpt::grad_cost_tracking_visibility(const Eigen::Vector3d& p,
  ...
  for (size_t k = 0; k < sample; k++){
    ...
    if (dist < threshold_k){
      double pen = threshold_k * threshold_k - dist * dist;
      double grad_L1 = 0;
      costp = smoothedL1(pen, grad_L1);
      gradp = grad_L1 * (2.0 * threshold_k * rho * (1-lambda_k) * d / d.norm() - 
