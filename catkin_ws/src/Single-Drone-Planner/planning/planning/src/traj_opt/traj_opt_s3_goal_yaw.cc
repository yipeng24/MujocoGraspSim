...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  auto traj_info = minco_s3_opt_.getTraj();
  for(int i = 1; i < N_; ++i){
...
inline double objectiveFuncGoal(void* ptrObj,
  ...
  obj.minco_s3_opt_.gdT.array() += obj.rhoT_;
  cost += obj.rhoT_ * T.sum();
  obj.cost_t_rec_ = obj.rhoT_ * T.sum();

  addLayerTGrad(t, obj.minco_s3_opt_.gdT + obj.minco_s2_yaw_opt_.gdT, gradt);
...
void TrajOpt::addTimeIntPenaltyGoal(double& cost){
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    for (int j = 0; j < innerLoop; ++j) {
      ...
      if(i > 0 && i < N_ - 1){ // ignore head and tail piece
...
void TrajOpt::addTimeCostForward(double& cost){
  ...
  for (int i = 0; i < M; ++i) {
    ...
    INFO_MSG_GREEN("M: "<<M<<" piece: "<<piece);
    if (piece > N_) INFO_MSG_RED("piece wrong!!!!!!!!!!!!!!!!!!!!!!!!!");
    double rho = 1.0;
    ...
    if(grad_cost_yaw_forward(yaw, vel, grad_tracking_yaw, grad_vel, cost_tracking_yaw)) 
    {
      grad_yaw += grad_tracking_yaw;
      grad_v += grad_vel;
      cost_inner += cost_tracking_yaw;
      // INFO_MSG("cost inner: " << cost_inner);
