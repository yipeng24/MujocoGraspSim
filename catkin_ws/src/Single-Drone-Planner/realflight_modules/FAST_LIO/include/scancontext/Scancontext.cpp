...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        double candidate_dist = sc_dist_result.first;
        int candidate_align = sc_dist_result.second;
        cout << "id: " << candidate_indexes[candidate_iter_idx] << " candidate_dist: " << candidate_dist << endl;
        if( candidate_dist < min_dist )
        {
            min_dist = candidate_dist;
            nn_align = candidate_align;
...
std::pair<int, float> SCManager::detect_init_pose ( bool print_detail_score )
{
    int loop_id { -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")

    auto curr_desc = source_scancontext;
        
    double min_dist = 10000000; // init with somthing large
    int nn_align = 0;
    int nn_idx = 0;
  
    for ( int id = 0; id < polarcontexts_.size(); id++ )
    {
        MatrixXd polarcontext_candidate = polarcontexts_[ id ];
        std::pair<double, int> sc_dist_result = distanceBtnScanContext( polarcontext_candidate , curr_desc); 
        
        double candidate_dist = sc_dist_result.first;
        int candidate_align = sc_dist_result.second;
        if(print_detail_score)
            cout << "id: " << id << " candidate_dist: " << candidate_dist << endl;
        if( candidate_dist < min_dist )
        {
            min_dist = candidate_dist;
            nn_align = candidate_align;
