...
void Preprocess::Lidar_wrt_Body_RT(Eigen::Vector3d T, Eigen::Matrix3d R)
{
  Lidar_wrt_Body_T = T;
  Lidar_wrt_Body_R = R;
}
...
{  
  ...
  {
    Eigen::Vector3d pt{pl_surf.points[i].x, pl_surf.points[i].y, pl_surf.points[i].z};
    pt = Lidar_wrt_Body_R * pt + Lidar_wrt_Body_T;
    pl_surf.points[i].x = pt[0];
    pl_surf.points[i].y = pt[1];
    pl_surf.points[i].z = pt[2];
  }
  *pcl_out = pl_surf;
}
