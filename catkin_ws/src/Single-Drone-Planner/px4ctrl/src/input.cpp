...
Command_Data_t::Command_Data_t() {
  rcv_stamp = last_grab_cmd_stamp = ros::Time(0);
}
...
void Command_Data_t::feed(quadrotor_msgs::PositionCommandConstPtr pMsg) {
  ...
  // * airgrasp
  if (msg.theta.position.size() < 3) {
    // std::cout << "position_cmd theta size < 3" << std::endl;
    return;
  }

  theta(0) = msg.theta.position[0];
  theta(1) = msg.theta.position[1];
  theta(2) = msg.theta.position[2];

  dtheta(0) = msg.theta.velocity[0];
  dtheta(1) = msg.theta.velocity[1];
  dtheta(2) = msg.theta.velocity[2];

  if (msg.grab_cmd != grab_cmd) {
    grab_cmd = msg.grab_cmd;
    last_grab_cmd_stamp = rcv_stamp;
  }
...
void Battery_Data_t::feed(sensor_msgs::BatteryStateConstPtr pMsg) {
  ...
  if (percentage > 0.05) {
    ...
    if ((rcv_stamp - last_print_t).toSec() > 1) {
      ROS_ERROR("[px4ctrl] Dangerous! voltage=%.3f, percentage=%.3f", volt,
                percentage);
      last_print_t = rcv_stamp;
    }
  }
...
// * airgrasp
Joint_State_Data_t::Joint_State_Data_t() { rcv_stamp = ros::Time(0); }

void Joint_State_Data_t::feed(sensor_msgs::JointStateConstPtr pMsg) {
  msg = *pMsg;

  theta(0) = msg.position[0];
  theta(1) = msg.position[1];
  theta(2) = msg.position[2];
  dtheta(0) = msg.velocity[0];
  dtheta(1) = msg.velocity[1];
  dtheta(2) = msg.velocity[2];

  rcv_stamp = ros::Time::now();
}

Joy_Data_t::Joy_Data_t() { rcv_stamp = last_grab_cmd_stamp = ros::Time(0); }

#define BUTTON_GRAB_OPEN 2
#define BUTTON_GRAB_CLOSE 3
void Joy_Data_t::feed(sensor_msgs::JoyConstPtr pMsg) {
  msg = *pMsg;
  rcv_stamp = ros::Time::now();

  if (msg.buttons.at(BUTTON_GRAB_OPEN)) {
    grab_cmd = false;
    last_grab_cmd_stamp = rcv_stamp;
  } else if (msg.buttons.at(BUTTON_GRAB_CLOSE)) {
    grab_cmd = true;
    last_grab_cmd_stamp = rcv_stamp;
  }
}
