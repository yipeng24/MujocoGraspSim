...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param_, Controller &controller_)
...
void PX4CtrlFSM::process() {
  ...
  Desired_State_t des(odom_data, theta_cur_data);
  ...
  case MANUAL_CTRL: {
    if (rc_data.enter_hover_mode) // Try to jump to AUTO_HOVER
    {
      ...
      if (!theta_fb_is_received(now_time)) {
        ROS_ERROR("[px4ctrl] Reject AUTO_HOVER(L2). No joint state feedback!");
        break;
      }
      // if (!joy_cmd_is_received(now_time))
      // {
      // 	ROS_ERROR("[px4ctrl] Reject AUTO_HOVER(L2). No joy cmd!");
      // 	break;
      // }
  ...
  case AUTO_HOVER: {
    if (!rc_data.is_hover_mode || !odom_is_received(now_time)) {
      ...
      if (state_data.current_state.mode == "OFFBOARD") {
        ...
        // std::cout << "[AUTO_HOVER -> CMD_CTRL] des.thetas:" << des.theta.transpose() << std::endl;
      ...
      // std::cout << "[AUTO_HOVER] des.thetas:" << des.theta.transpose() << std::endl;
  ...
  case CMD_CTRL: {
    if (!rc_data.is_hover_mode || !odom_is_received(now_time)) {
      state = MANUAL_CTRL;
      toggle_offboard_mode(false); // move to px4ctrl
      ...
      // std::cout << "[CMD_CTRL->AUTO_HOVER] des.thetas:" << des.theta.transpose() << std::endl;

      ROS_INFO("[px4ctrl] From CMD_CTRL(L3) to AUTO_HOVER(L2)!");
    } else {
      des = get_cmd_des();

      // std::cout << "[CMD_CTRL] des.thetas:" << des.theta.transpose() << std::endl;
    }
  ...
  if (rotor_low_speed_during_land) // used at the start of auto takeoff
  {
    ...
    case 0:
      debug_msg =
          controller.update_alg0(des, odom_data, imu_data, u, bat_data.volt);
    ...
    case 1:
      debug_msg =
          controller.update_alg1(des, odom_data, imu_data, u, bat_data.volt);
      debug_msg.header.stamp = now_time;
      debug_pub.publish(debug_msg);
      break;
    ...
    case 3: {
      debug_msg =
          controller.update_alg3(des, odom_data, theta_cur_data, imu_data,
                                 state == CMD_CTRL, u, bat_data.volt);
      debug_msg.header.stamp = now_time;
      debug_pub.publish(debug_msg);
      end_pose_ref_pub.publish(debug_msg.end_pose_ref);
      break;
    }
...
Desired_State_t PX4CtrlFSM::get_hover_des() {
  ...
  des.theta = hover_theta;
  des.dtheta = Eigen::Vector3d::Zero();
...
Desired_State_t PX4CtrlFSM::get_cmd_des() {
  ...
  des.theta = cmd_data.theta;
  des.dtheta = cmd_data.dtheta;

  return des;
}

void PX4CtrlFSM::set_theta_manul_des(Desired_State_t des) {
  des.theta.resize(theta_cmd_manual_data.theta.size());
  des.dtheta.resize(theta_cmd_manual_data.dtheta.size());

  for (size_t i = 0; i < theta_cmd_manual_data.theta.size(); ++i) {
    des.theta(i) = theta_cmd_manual_data.theta[i];
    des.dtheta(i) = theta_cmd_manual_data.dtheta[i];
  }
}
...
void PX4CtrlFSM::set_hov_with_odom() {
  ...
  hover_theta << theta_cur_data.theta[0], theta_cur_data.theta[1],
      theta_cur_data.theta[2];
...
void PX4CtrlFSM::set_hov_with_rc() {
  ...
  thetas_add << joy_cmd_data.msg.axes.at(CH_PITCH),
      joy_cmd_data.msg.axes.at(CH_PITCH2), joy_cmd_data.msg.axes.at(CH_ROLL);
  hover_theta += thetas_add * delta_t;
  thetasLimit(hover_theta);
...
void PX4CtrlFSM::thetasLimit(Eigen::Vector3d &thetas) {
  Eigen::Vector3d thetas_low, thetas_high;
  thetas_low << 0, -M_PI / 2, -0.8*M_PI;
  thetas_high << M_PI / 2, M_PI / 2, 0.8*M_PI;

  for (int i = 0; i < 3; ++i) {
    if (thetas[i] < thetas_low[i])
      thetas[i] = thetas_low[i];
    if (thetas[i] > thetas_high[i])
      thetas[i] = thetas_high[i];
  }
}
...
// airgrasp
bool PX4CtrlFSM::theta_fb_is_received(const ros::Time &now_time) {
  return (now_time - theta_cur_data.rcv_stamp).toSec() <
         param.msg_timeout.theta;
}

bool PX4CtrlFSM::joy_cmd_is_received(const ros::Time &now_time) {
  return (now_time - joy_cmd_data.rcv_stamp).toSec() < param.msg_timeout.theta;
}
...
void PX4CtrlFSM::publish_bodyrate_ctrl(const Controller_Output_t &u,
  ...
  joint_msg.name = theta_cur_data.msg.name;
  joint_msg.position.resize(u.theta.size());
  joint_msg.velocity.resize(u.dtheta.size());

  for (size_t i = 0; i < u.dtheta.size(); ++i) {
    joint_msg.position[i] = u.theta(i);
    joint_msg.velocity[i] = u.dtheta(i);
  }
  theta_cmd_pub.publish(joint_msg);
}
...
void PX4CtrlFSM::joint_state_est_cb(
    const sensor_msgs::JointState::ConstPtr &msg) {
  theta_cur_data.feed(msg);

  Eigen::Vector3d e_pos;
  Eigen::Quaterniond e_quat;
  Eigen::Matrix4d T_e2b;
  rc_sdf_ptr->kine_ptr_->getEndPose(odom_data.p, odom_data.q,
                                    theta_cur_data.theta, e_pos, e_quat, T_e2b);
  ...
  end_pose_est_pub.publish(end_pose_msg);
}

void PX4CtrlFSM::position_cmd_grab_cb(
    const quadrotor_msgs::PositionCommandConstPtr &pMsg) {
  if (pMsg->grab_cmd != 2) {
    grab_cmd_queue.push(pMsg->grab_cmd);
  }

  cmd_data.feed(pMsg);
}

void PX4CtrlFSM::joy_grab_cb(const sensor_msgs::JoyConstPtr &pMsg) {
#define BUTTON_GRAB_OPEN 2
#define BUTTON_GRAB_CLOSE 3

  if (pMsg->buttons.size() > BUTTON_GRAB_CLOSE) {
    if (pMsg->buttons.at(BUTTON_GRAB_OPEN)) {
      grab_cmd_queue.push(0); // open
    } else if (pMsg->buttons.at(BUTTON_GRAB_CLOSE)) {
      grab_cmd_queue.push(1); // close
    }
  }
  joy_cmd_data.feed(pMsg);
}

void PX4CtrlFSM::pub_grab_cmd() {
  if (!grab_cmd_queue.empty()) {
    uint8_t cmd = grab_cmd_queue.front();
    grab_cmd_queue.pop();

    std_msgs::Bool grab_msg;
    grab_msg.data = (cmd == 1); // 0->false (open), 1->true (close)
    grab_cmd_pub.publish(grab_msg);
  }
}
