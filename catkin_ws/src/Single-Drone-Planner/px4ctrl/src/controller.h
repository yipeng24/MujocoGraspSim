...
struct Desired_State_t {
  ...
  Desired_State_t(Odom_Data_t &odom, Joint_State_Data_t &theta)
      : p(odom.p), v(Eigen::Vector3d::Zero()), a(Eigen::Vector3d::Zero()),
        ...
        theta(theta.theta), dtheta(theta.dtheta) {};
};
...
struct L1Block {
  // disturbance estimate in world frame
  Eigen::Vector3d d_hat = Eigen::Vector3d::Zero();      // fast adaptation state
  Eigen::Vector3d d_hat_filt = Eigen::Vector3d::Zero(); // low-pass filtered
  Eigen::Vector3d a_lpf =
      Eigen::Vector3d::Zero(); // low-pass filtered acceleration
  bool inited = false;
};
...
public:
  ...
  update_alg0(const Desired_State_t &des, const Odom_Data_t &odom,
              const Imu_Data_t &imu, Controller_Output_t &u, double voltage);

  Eigen::Vector3d
  computeLimitedTotalAccFromThrustForce(const Eigen::Vector3d &thrustforce,
  ...
  bool flatnessWithDrag(const Eigen::Vector3d &vel, const Eigen::Vector3d &acc,
                        ...
                        Eigen::Vector3d &omg, const double &mass,
                        const double &grav, const double &dh, const double &dv,
                        const double &cp, const double &veps) const;

  void minimumSingularityFlatWithDrag(
  ...
  quadrotor_msgs::Px4ctrlDebug
  update_alg1(const Desired_State_t &des, const Odom_Data_t &odom,
              const Imu_Data_t &imu, Controller_Output_t &u, double voltage);
  ...
  void computeFlatInput(const Eigen::Vector3d &thr_acc,
                        ...
                        const double &yawd, const Eigen::Quaterniond &att_est,
                        Eigen::Quaterniond &att, Eigen::Vector3d &omg) const;

  /*Algorithm from the rotor-drag paper*/
  void update_alg2(const Desired_State_t &des, const Odom_Data_t &odom,
                   const Imu_Data_t &imu, Controller_Output_t &u,
                   double voltage);

  void computeAeroCompensatedReferenceInputs(const Desired_State_t &des,
  ...
  Eigen::Quaterniond
  computeDesiredAttitude(const Eigen::Vector3d &des_acc,
  ...
  //* jr revised
  quadrotor_msgs::Px4ctrlDebug
  update_alg3(const Desired_State_t &des, const Odom_Data_t &odom,
              const Joint_State_Data_t &js, const Imu_Data_t &imu,
              const bool &in_cmd, Controller_Output_t &u,
              double voltage);

  Eigen::Vector3d computeFeedBackControlTheta(const Eigen::Quaterniond &des_q,
                                              const Eigen::Quaterniond &est_q,
                                              const Parameter_t &param);

  /* Shared functions*/
  Eigen::Vector3d computePIDErrorAcc(const Odom_Data_t &odom,
  ...
  Eigen::Vector3d
  computeLimitedAngularAcc(const Eigen::Vector3d candirate_bodyrate);

  double computeDesiredCollectiveThrustSignal(const Eigen::Quaterniond &est_q,
                                              const Eigen::Vector3d &est_v,
                                              const Eigen::Vector3d &des_acc,
                                              const Parameter_t &param,
                                              double voltage);

  double AccurateThrustAccMapping(const double des_acc_z, double voltage,
                                  const Parameter_t &param) const;

  Eigen::Vector3d
  computeFeedBackControlBodyrates(const Eigen::Quaterniond &des_q,
                                  const Eigen::Quaterniond &est_q,
                                  const Parameter_t &param);

  bool estimateThrustModel(const Eigen::Vector3d &est_a, const double voltage,
                           const Eigen::Vector3d &est_v,
                           const Parameter_t &param);
...
private:
  ...
  // L1
  L1Block l1_;
  Eigen::Vector3d last_nominal_acc_ =
      Eigen::Vector3d::Zero(); // 记录未补偿的名义加速度

	Eigen::VectorXd calculateArmCompensation(
		...
		const Eigen::Vector3d &w_p, const Eigen::Vector3d &w_r);

  void clipTheta(Eigen::VectorXd &theta, const Parameter_t &param) const;

  void applyL1Compensation(Eigen::Vector3d &total_des_acc,
                           const Odom_Data_t &odom, const Imu_Data_t &imu);

  // 在 controller.h 的 private 区域添加
  struct {
      bool inited = false;
