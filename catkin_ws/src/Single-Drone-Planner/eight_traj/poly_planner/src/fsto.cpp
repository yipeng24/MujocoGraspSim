...
MavGlobalPlanner::MavGlobalPlanner(Config &conf, NodeHandle &nh_)
  ...
  triggerSub =
      nh.subscribe(config.triggerTopic, 1, &MavGlobalPlanner::triggerCallBack,
                   this, TransportHints().tcpNoDelay());
  odomSub = nh.subscribe(config.odomTopic, 1, &MavGlobalPlanner::odomCallBack,
                         this, TransportHints().tcpNoDelay());
  trajPub = nh.advertise<quadrotor_msgs::PolynomialTrajectory>(
      config.trajectoryTopic, 1);
}

void MavGlobalPlanner::triggerCallBack(
  ...
  double tanGamma =
      (startTrans(1) - xyzMin(1)) / (xyzMax(0) - startTrans(0) + FLT_EPSILON);
  ...
  Eigen::MatrixXd box(6, 6), inic(6, 8), lbrt(6, 8), rtlt(6, 7), ltrb(6, 8),
      rblb(6, 7);
  box << Eigen::Vector3d::UnitX(), Eigen::Vector3d::UnitY(),
      Eigen::Vector3d::UnitZ(), -Eigen::Vector3d::UnitX(),
      -Eigen::Vector3d::UnitY(), -Eigen::Vector3d::UnitZ(), xyzMax, xyzMax,
      xyzMax, xyzMin, xyzMin, xyzMin;
  inic << box.topRows<3>(), Eigen::Vector3d(sinGamma, cosGamma, 0),
      -Eigen::Vector3d(sinGamma, cosGamma, 0), box.bottomRows<3>(),
      Eigen::Vector3d(xyzMax(0), xyzMin(1) + yProjIniWidth, xyzMax(2)),
      Eigen::Vector3d(xyzMax(0) - xProjIniWidth, xyzMin(1), xyzMax(2));
  lbrt << box.topRows<3>(), Eigen::Vector3d(sinTheta, cosTheta, 0),
      -Eigen::Vector3d(sinTheta, cosTheta, 0), box.bottomRows<3>(),
      Eigen::Vector3d(xyzMax(0), xyzMin(1) + yProjWidth, xyzMax(2)),
      Eigen::Vector3d(xyzMax(0) - xProjWidth, xyzMin(1), xyzMax(2));
  rtlt << box.topRows<3>(), -Eigen::Vector3d::UnitX(), box.bottomRows<3>(),
      xyzMax - sideWidth * Eigen::Vector3d::UnitX();
  ltrb << box.topRows<3>(), Eigen::Vector3d(sinTheta, -cosTheta, 0),
      -Eigen::Vector3d(sinTheta, -cosTheta, 0), box.bottomRows<3>(),
      Eigen::Vector3d(xyzMax(0), xyzMax(1) - yProjWidth, xyzMax(2)),
      Eigen::Vector3d(xyzMax(0) - xProjWidth, xyzMax(1), xyzMax(2));
  rblb << box.topRows<3>(), Eigen::Vector3d::UnitX(), box.bottomRows<3>(),
      xyzMin + sideWidth * Eigen::Vector3d::UnitX();
  ...
  if (!nonlinOpt.setup(config.weightT, 1.0, iniState, finState, hPolys, res,
                       itg, vmax, amax, chi, config.c2Diffeo)) {
    return;
  }
...
void Visualization::visualize(const Trajectory &appliedTraj, Time timeStamp,
                              double compT) {
