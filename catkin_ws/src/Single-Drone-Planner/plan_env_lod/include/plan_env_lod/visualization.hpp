...
namespace visualization_airgrasp {
...
class Visualization {
 ...
 //         pair vector3d double; xyz itensity
  ...
  void visualize_pointcloud(const PC& pc, const TOPIC& topic) {
    // std::cout << "visualize_pointcloud 0" << std::endl;
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
      ros::Publisher pub = nh_.advertise<sensor_msgs::PointCloud2>(topic, 10);
      publisher_map_[topic] = pub;
    }
    // std::cout << "visualize_pointcloud 1" << std::endl;
    ...
    // std::cout << "visualize_pointcloud 2" << std::endl;
    pcl::toROSMsg(point_cloud, point_cloud_msg);
    point_cloud_msg.header.frame_id = "world";
    point_cloud_msg.header.stamp = ros::Time::now();
    publisher_map_[topic].publish(point_cloud_msg);
    // std::cout << "visualize_pointcloud 3" << std::endl;
  }
  ...
  void visualize_balls(const BALLS& balls,
    ...
    marker.action = visualization_msgs::Marker::ADD;
    int n_pts = balls.cols();
    for (int i=0; i<n_pts; ++i) {
      Eigen::VectorXd ball = balls.col(i);
      setMarkerPose(marker, ball[0], ball[1], ball[2]);
      auto d = 2 * ball[3];
      setMarkerScale(marker, d, d, d);
      marker.color.a = ball[4];
      marker_array.markers.push_back(marker);
      marker.id++;
    }
  ...
  void visualize_pairline(const PAIRLINE& pairline, const TOPIC& topic, const Color color = yellow) {
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
      ros::Publisher pub =
          nh_.advertise<visualization_msgs::MarkerArray>(topic, 10);
  ...
  void visualize_arrows(const ARROWS& arrows, const TOPIC& topic, visualization_airgrasp::Color color = yellow) {
