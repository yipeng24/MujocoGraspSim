...
__global__ void render(float3 *data_devptr, Parameter *para_devptr, DeviceImage<int> *depth_devptr)
{
	...
	int r = para.splat_k * para.fx / dist + 0.5f;
	// int r = 0.005 * para.fx / dist + 0.5f;
...
void DepthRender::set_para(float _fx, float _fy, float _cx, float _cy, int _width, int _height, float _splat_k)
{
	parameter.fx = _fx;
	parameter.fy = _fy;
	parameter.cx = _cx;
