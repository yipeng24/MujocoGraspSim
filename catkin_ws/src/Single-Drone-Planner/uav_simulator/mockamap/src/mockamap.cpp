...
main(int argc, char** argv)
{
  ...
  mocka::Maps map;
  std::cout << "[mockamap] start setting info" << std::endl;
  map.setInfo(info);
  std::cout << "[mockamap] start generating map" << std::endl;
  map.generate(type);
  std::cout << "[mockamap] finish generating map" << std::endl;
  ...
  while (ros::ok())
  {
    ...
    // std::cout << "[mockamap] publish" << std::endl;
    ros::spinOnce();
    // std::cout << "[mockamap] publish2" << std::endl;
    loop_rate.sleep();
    // std::cout << "[mockamap] publish3" << std::endl;
  }
  return 0;
}
