...
# from cv_bridge import CvBridge   # <-- 移除
import ros_numpy as rnp            # <-- 新增
...
def image_to_numpy(msg: Image, want_bgr=True) -> np.ndarray:
    """
    使用 ros_numpy 将 ROS Image 转为 numpy。
    - 如果是彩色图：支持 rgb8 / bgr8；可选转换为 BGR（OpenCV/Ultralytics 常用）。
    - 如果是深度图：返回原始 dtype（uint16 for 16UC1 / float32 for 32FC1）。
    """
    arr = rnp.numpify(msg)  # 形状 (H,W,C) 或 (H,W)
...
def numpy_to_image(arr: np.ndarray, header: Header, encoding: str) -> Image:
    """
    使用 ros_numpy 将 numpy 转为 ROS Image。
    encoding 例如：'bgr8'、'mono8'、'32FC1' 等，需与 arr 的 dtype/形状匹配。
    """
...
class YOLO11SegFromSyncFramePack:
    def __init__(self):
        ...
        # self.bridge = CvBridge()  # <-- 移除 CvBridge
    ...
    def cb_syncframe(self, sf: SyncFrame):
        # 1) 解码 RGB（到 BGR numpy）
        try:
            bgr = image_to_numpy(sf.sync_rgb, want_bgr=True)  # BGR uint8
            if bgr.dtype != np.uint8:
                bgr = bgr.astype(np.uint8, copy=False)
        except Exception as e:
            rospy.logwarn(f"[PackNode] rosnumpy rgb failed: {e}")
            return

        # 2) 解码深度（米）
        try:
            enc = sf.sync_depth.encoding if self.depth_encoding == "auto" else self.depth_encoding
            depth_np = rnp.numpify(sf.sync_depth)  # uint16 (16UC1) or float32 (32FC1)
            if enc == "16UC1" or ("16uc1" in sf.sync_depth.encoding.lower() and self.depth_encoding == "auto"):
                depth_m = depth_np.astype(np.float32) * self.depth_scale
            else:
                # 假定 32FC1（米）
                depth_m = depth_np.astype(np.float32, copy=False)
        except Exception as e:
            rospy.logwarn(f"[PackNode] rosnumpy depth failed: {e}")
            return
    ...
    def _publish_syncframe_out(self, sf_in: SyncFrame, cloud_fg: PointCloud2, cloud_bg: PointCloud2):
        if self.pub_sync_out.get_num_connections() == 0:
            return
        out = SyncFrame()
        out.header = sf_in.header
        out.sync_depth = sf_in.sync_depth
        out.sync_rgb = sf_in.sync_rgb
        out.sync_tgt_pcl = cloud_fg
    ...
    def _publish_mask_and_overlay(self, header, bgr, fg_mask):
        # mask
        try:
            mask_msg = numpy_to_image(fg_mask, header, encoding="mono8")
        ...
        if self.pub_overlay.get_num_connections() > 0:
            try:
                ...
                overlay_msg = numpy_to_image(overlay, header, encoding="bgr8")
